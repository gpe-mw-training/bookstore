...
public class CatalogServiceTest extends BaseWebTestTemplate {
	...
	public void testByCategory() {
		List<CatalogItem> items = service.lookupItemsByCategory("children", false);
		Assert.assertNotNull(items);
	...
	public void testByCategoryNewItems() {
		List<CatalogItem> items = service.lookupItemsByCategory("children", true);
		Assert.assertNotNull(items);
	...
	public void testSearchCategory() {
		...
		Assert.assertNotNull(items);
		Assert.assertEquals(1, items.size());
		Assert.assertEquals("The Boy Scouts at the Panama-Pacific Exposition", items.get(0).getTitle());
	}
	...
	public void testSearchDescription() {
		...
		Assert.assertNotNull(items);
		Assert.assertEquals(1, items.size());
		Assert.assertEquals("The Adventures of Maya the Bee", items.get(0).getTitle());
	}
}
